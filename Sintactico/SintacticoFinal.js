...
function valida_distinct(posicion) {
    ...
    if (DATA[posicion + 1] === 10) { // Corregido para verificar el valor 10
        console.log("Token DISTINCT encontrado");
        posicion = posicion + 2; // Mover la posición más allá del token DISTINCT
    }
...
function valida_order_by(posicion) {
    ...
    // Verifica que haya al menos un elemento después de ORDER BY
    if (DATA[posicion + 1] !== undefined) {
        console.log("Validando ordenamiento");

        // Valida la primera parte del ordenamiento
        if (DATA[posicion + 1] === ASC || DATA[posicion + 1] === DESC) {
            posicion += 2; // Moverse más allá de ASC o DESC

            // Puedes agregar más lógica para validar otros elementos de ordenamiento aquí
        } else {
...
for (i = 0; i < DATA.length; i++) {
    if (DATA[i] == 200) {
        ...
        // No llames a valida_where aquí, ya que ya se llamó dentro de valida_select
        // i = valida_where(i);
